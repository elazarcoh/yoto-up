...
from yoto_up_server.dependencies import ApiServiceDep, ContainerDep
...
async def auth_status(request: Request, api_service: ApiServiceDep) -> str:
    ...
    return render_partial(
        AuthStatusPartial(is_authenticated=is_authenticated)
    )
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                return render_partial(
                    AuthStatusPartial(
                        is_authenticated=False,
                        error=f"Authentication failed: Unexpected response from server",
                    )
                )
...
async def oauth_callback(request: Request, api_service: ApiServiceDep, code: str = None, state: str = None, error: str = None):
    """
    ...
    try:
        ...
        if not state or state not in request.app.state.oauth_states:
            logger.warning(f"State mismatch in OAuth callback")
            return RedirectResponse(
                url="/auth/?error=invalid_state",
                status_code=302
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                ...
                return RedirectResponse(
                    url="/auth/?error=token_exchange_failed",
                    status_code=302
                )
            ...
            if not access_token:
                ...
                return RedirectResponse(
                    url="/auth/?error=no_access_token",
                    status_code=302
                )
            
            # Save tokens
            api_service.save_tokens(access_token, refresh_token, id_token)
            logger.info(f"OAuth authentication successful, tokens saved")
        ...
        return RedirectResponse(
            url="/auth/?error=http_error",
            status_code=302
        )
    except Exception as e:
        logger.error(f"Error handling OAuth callback: {e}")
        return RedirectResponse(
            url="/auth/?error=callback_error",
            status_code=302
        )
