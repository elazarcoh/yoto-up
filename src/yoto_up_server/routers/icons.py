...
from yoto_up_server.dependencies import IconServiceDep
from yoto_up_server.templates.icon_components import IconImg
...
async def get_icon_media(
    ...
    """
    try:
        logger.info(f"Fetching icon: {media_id}")
        
        # Try to get the icon (this downloads if not cached)
        icon_bytes = await icon_service.get_icon_by_media_id(media_id)
